...
function Signup({ darkMode }) {
  ...
  const handleChange = (e) => {
    ...
    // Reset consultant-specific fields when switching roles
    if (name === 'role' && value !== 'consultant') {
      setConsultantExtra({ bio: '', specialties: '', fee: '' });
      setImageFile(null);
    }
  };
  ...
  const handleSignup = async (e) => {
    ...
    // Extra validation for consultant
    if (role === 'consultant' && (!consultantExtra.bio || !consultantExtra.fee)) {
      return handleError('Please fill all consultant details');
    }
